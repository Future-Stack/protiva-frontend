...
import {
    ...
    useGetAllCategoriesQuery, useGetSubCategoriesQuery, useUpdateCategoryMutation,
} from "@/lib/features/super-admin/category/categoryAPI";
import {
...
export default function CategoriesPage() {
    ...
    const hasViewPermission = user?.role === "SUPER_ADMIN" || user?.adminPermissions?.isViewCategory || user?.adminPermissions?.isManageCategory;
    ...
    const handleSubmit = async () => {
        if (!categoryName) { Swal.fire({ icon: "warning", title: "Required", text: "Category name is required." }); return; }
        ...
        try {
            if (editingCategory) {
                ...
                Swal.fire({ icon: "success", title: "Created", text: `"${result.data?.name}" added.`, timer: 2000, showConfirmButton: false });
            }
            handleReset();
        } catch (err: any) {
            Swal.fire({ icon: "error", title: "Failed", text: err?.data?.message || "Something went wrong." });
        }
    };

    const handleReset = () => {
        setCategoryName(""); setDescription(""); setImageFile(null); setIconFile(null);
        setImagePreview(null); setIconPreview(null); setEditingCategory(null);
    };

    const handleEdit = (cat: CategoryItem) => {
        setCategoryName(cat.name); setDescription(cat.description);
        setImagePreview(cat.image); setIconPreview(cat.icon); setEditingCategory(cat);
    ...
    const confirmDelete = async () => {
        if (itemToDelete) {
            try { await deleteCategory(itemToDelete).unwrap(); Swal.fire("Deleted!", "Category removed.", "success"); }
            catch (err: any) { Swal.fire("Error", err?.data?.message || "Failed.", "error"); }
            setItemToDelete(null);
        }
        setIsDeleteModalOpen(false);
    };

    const handleAddSubCategory = (id: string) => { setSelectedCategoryId(id); setSelectedSubCategory(null); setIsSubModalOpen(true); };
    const handleEditSubCategory = (sub: SubCategoryItem) => { setSelectedSubCategory(sub); setSelectedCategoryId(sub.categoryId); setIsSubModalOpen(true); };
    const handleDeleteSubCategory = (id: string) => { setSubCategoryToDelete(id); setIsSubDeleteModalOpen(true); };
    const confirmDeleteSubCategory = async () => {
        if (subCategoryToDelete) {
            try { await deleteSubCategory(subCategoryToDelete).unwrap(); Swal.fire("Deleted!", "Sub-category removed.", "success"); }
            catch (err: any) { Swal.fire("Error", err?.data?.message || "Failed.", "error"); }
    ...
    const toggleExpand = (id: string) => setExpandedCategoryId(expandedCategoryId === id ? null : id);
        ...
        return (
            <div className="flex items-center justify-center h-[50vh]">
                <div className="text-center">
                    ...
                    <p className="text-slate-500">You do not have permission to view categories.</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="bg-white rounded-lg overflow-hidden">
                    <div className="p-4 sm:px-[55px] sm:py-[41px] space-y-6">
                        ...
                        <div className="mt-6 flex flex-wrap gap-8">
                            ...
                            {/* <div>
                                ...
                                <div onClick={() => imageInputRef.current?.click()}
                                    ...
                                    <div className="flex flex-col items-center gap-2">
                                        <h6 className="text-base font-semibold text-[#18181A] mb-2">Category Image</h6>
                                        <div className="flex flex-col items-center justify-center text-center">
                                            <div className="relative w-16 h-16 bg-slate-100 rounded-lg flex items-center justify-center mb-4">
                                                {imagePreview ? <img src={imagePreview} alt="preview" className="w-full h-full object-cover rounded-lg" /> : <><Upload className="w-8 h-8 text-slate-400" /><img src={ImageIcon2.src} alt="" /></>}
                                                <button type="button" onClick={(e) => { e.stopPropagation(); imageInputRef.current?.click(); }}
                                                    className="absolute -bottom-5 -right-2 w-4 h-4 flex items-center justify-center bg-[#4153B395] hover:bg-primary/80 text-white rounded-full transition-colors mb-3">
                                                    <Pencil className="w-2 h-2" />
                                                </button>
                                            </div>
                                            <p className="text-xs text-[#5E6472] opacity-75">(jpg, png, jpeg - Max 2MB)</p>
                                        </div>
                                    </div>
                                </div>
                            </div> */}
                            ...
                            <div>
                                ...
                                <div onClick={() => iconInputRef.current?.click()}
                                    className="w-fit border-2 border-dashed border-slate-300 rounded-lg px-10 py-6 text-center hover:border-blue-400 transition-colors cursor-pointer">
                                    <div className="flex flex-col items-center gap-2">
                                        <h6 className="text-base font-semibold text-[#18181A] mb-2">Category Icon</h6>
                                        <div className="flex flex-col items-center justify-center text-center">
                                            <div className="relative w-16 h-16 bg-slate-100 rounded-lg flex items-center justify-center mb-4">
                                                {iconPreview ? <img src={iconPreview} alt="preview" className="w-full h-full object-cover rounded-lg" /> : <><Upload className="w-8 h-8 text-slate-400" /><img src={ImageIcon2.src} alt="" /></>}
                                                <button type="button" onClick={(e) => { e.stopPropagation(); iconInputRef.current?.click(); }}
                                                    className="absolute -bottom-5 -right-2 w-4 h-4 flex items-center justify-center bg-[#4153B395] hover:bg-primary/80 text-white rounded-full transition-colors mb-3">
                                                    <Pencil className="w-2 h-2" />
                                                </button>
                                            </div>
                                            <p className="text-xs text-[#5E6472] opacity-75">(svg, png, jpg - Max 2MB)</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-wrap justify-center md:justify-start items-center gap-3 mt-10">
                            ...
                            <button onClick={handleReset} className="px-20 py-2.5 bg-white border border-slate-300 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors">Reset</button>
                        </div>
                    </div>
                </div>
            )}
                                    ...
                                    displayedCategories.map((cat: CategoryItem, index: number) => (
                                        <Fragment key={cat.id}>
                                            <tr className={`border-t border-slate-300 hover:bg-slate-50/50 transition-colors ${expandedCategoryId === cat.id ? "bg-slate-50" : ""}`}>
                                                ...
                                                <td className="px-4 py-4 text-sm text-[#2C2C2C] border-r border-slate-300 text-center" onClick={() => toggleExpand(cat.id)}>
                                                    <div className="flex items-center justify-center gap-2">
                                                        <span className="">
                                                            <SubCategoryCount categoryId={cat.id} />
                                                            <span className="text-[10px] text-slate-400 font-medium hover:text-primary/90 cursor-pointer transition-colors hover:underline ml-1">
                                                                (Click to view)
                                                            </span>
                                                        </span>
                                                    </div>
            ...
            <DeleteModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={confirmDelete}
                title="Delete Category" description="Are you sure you want to delete this category? This action cannot be undone." />
            <SubCategoryModal isOpen={isSubModalOpen} onClose={() => setIsSubModalOpen(false)} categoryId={selectedCategoryId} subCategory={selectedSubCategory} />
            <DeleteModal isOpen={isSubDeleteModalOpen} onClose={() => setIsSubDeleteModalOpen(false)} onConfirm={confirmDeleteSubCategory}
                title="Delete Sub-category" description="Are you sure you want to delete this sub-category? This action cannot be undone." />
        </div>
    );
}
